...
template <typename R>
class ReturnPacket : public Serializable
{
public:
    using code_t = short;
    ...
    template <typename X>
    static std::ostream& Serialize(std::ostream &os, const ReturnPacket<X> &retPack)
    {
    ...
    template <typename X>
    static std::istream& DeSerialize(std::istream &is, ReturnPacket<X> &retPack)
    {
        size_t len;
        std::string temp;
        is >> retPack.code >> len;
